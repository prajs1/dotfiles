...
local function get_bundles()
	...
	local java_debug_path = get_path("java-debug-adapter")
	...
	local java_test_path = get_path("java-test")
...
local function get_workspace()
	...
	-- Declare a directory where project information will be stored
	local workspace_path = home .. "/code/workspace"
	--Determine the project name
	local project_name = vim.fn.fnamemodify(vim.fn.getcwd(), ":p:h:t")
	-- Create the workspace directory by concatenating the designed worksapce path and the project name
...
local function java_keymaps()
	...
	-- Set a Vim motion to <Space> + <Shift>J + t to run the test method is currently selected in visual mode
	vim.keymap.set(
		"v",
		"<leader>Jt",
...
local function setup_jdtls()
	-- Get access to the jdtls plugin an dall of its functionality
	local jdtls = require("jdtls")

	-- Get the paths to the jdtls jar, operating system specific configuration directory and Lombok jar
	local launcher, os_config, lombok = get_jdtls()

	-- Get the path that was specified to hold project information
	...
	-- Tell JDTLS language features it is capable of
	local capabilities = {
		workspace = {
			configuration = true,
		},
	...
	local settings = {
		java = {
			...
			-- Enable downloading archivs from eclipse automatically
			ecplise = {
				downloadSource = true,
			},
			-- Enable downloading erchives from maven automatically
			maven = {
			...
			-- Use the fernflower decompiler when using javap command to decompile byte code back to java code
			contentProvider = {
				preferred = "fernflower",
			},
			-- Set automatical package import organiztion on file save
			saveActions = {
				organizeImports = false,
			},
			-- Customize completion options
			completion = {
				-- When using an unimported static method, how should the LSP rankpossible places to import the static method from
			...
			codeGeneration = {
				...
				-- When generating hashCode and equals methodsuse the java 7 objects method
				hashCode = {
	...
	-- Create a table called init_options to pass the bundles withh debug and testing jar, along with the extended client capabilities to the start or attach function of JDTLS
	local init_options = {
		bundles = bundles,
	...
	local on_attach = function(_, bufnr)
		...
		-- Setup a function that automatically runs every time a java file is saved to refreshthe code lens
